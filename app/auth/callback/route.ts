...
export async function GET(request: NextRequest) {
  ...
  if (role === 'admin') {
    const path =
      nextAllowed && (nextAllowed === '/admin' || nextAllowed.startsWith('/admin/'))
        ? nextAllowed
        : '/admin'
    return redirectWithAuthCookies(`${baseUrl}${path}`, mergedCookies)
  }
  if (role === 'host') {
    const path =
      nextAllowed && (nextAllowed === '/host' || nextAllowed.startsWith('/host/'))
        ? nextAllowed
        : '/host'
    return redirectWithAuthCookies(`${baseUrl}${path}`, mergedCookies)
  }
  ...
  const resolveParticipantNext = (): string | null => {
    if (!nextAllowed) return null
    if (nextAllowed === '/auth/reset-password') return '/auth/reset-password'
    if (nextAllowed === '/admin' || nextAllowed.startsWith('/admin/')) return null
    if (nextAllowed === '/host' || nextAllowed.startsWith('/host/')) return null
    return nextAllowed
  }

  const effectiveNext = resolveParticipantNext()

  if (effectiveNext) {
    return redirectWithAuthCookies(`${baseUrl}${effectiveNext}`, mergedCookies)
  }

  return redirectWithAuthCookies(`${baseUrl}/dashboard`, mergedCookies)
}
