...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!p1Email) {
      return NextResponse.json(
        { error: 'Participant 1 has no email on file; cannot send notification.' },
        { status: 400 },
      )
    }
    ...
    const { subject, html, text } = buildP1PendingPartnerReminderEmail({
      ...
      p2InvitedEmail: team.p2_invited_email,
      invitationLink,
      updateP2EmailLink,
    ...
    return NextResponse.json({ success: true, sentTo: p1Email })
  } catch (error: unknown) {
    console.error('Error notifying P1 (pending partner):', error)
