...
export type DashboardClientProps = {
  ...
  initialParticipant: DashboardParticipantRow
  initialTeammate: DashboardTeammateRow | null
  initialAvailableExamsCount: number
  initialShowInvitationBanner?: boolean
}
...
export function DashboardClient({
  ...
  initialParticipant,
  initialTeammate,
  initialAvailableExamsCount,
  initialShowInvitationBanner = false,
}: DashboardClientProps) {
  const router = useRouter()
  const user = initialUser
  const [participantData, setParticipantData] = useState<DashboardParticipantRow>(initialParticipant)
  const teammateData = initialTeammate
  const availableExamsCount = initialAvailableExamsCount
  ...
  useEffect(() => {
    if (!participantData.profile_completed && typeof window !== 'undefined') {
      ...
      if (!hasModalBeenShown) {
        // eslint-disable-next-line react-hooks/set-state-in-effect -- localStorage is client-only; avoids hydration mismatch
        setShowProfileModal(true)
      }
    }
  }, [participantData.profile_completed, user.id])

  useEffect(() => {
    if (typeof window === 'undefined' || sessionStorage.getItem(EXAM_STATUS_SYNC_KEY)) return
  ...
  const handleProfileComplete = async () => {
    ...
    if (participant) {
      setParticipantData(participant as unknown as DashboardParticipantRow)
  ...
  const handleSaveAuthority = async () => {
    ...
    const { data: participant } = await supabase
      ...
      .eq('user_id', uid)
      .single()
    if (participant) setParticipantData(participant as unknown as DashboardParticipantRow)
  }
